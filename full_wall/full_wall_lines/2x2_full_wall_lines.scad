...
module 2x2_full_wall_line(sc, seed, manifold=-0.05) {
...
module 2x2_static_full_wall_line(front, back, right, left, manifold=-0.05) {
...
module 2x2_stagger_full_wall_line(sc, seed, manifold=-0.05) {
  seed_vect = rands(0, 100, 22, seed=seed);
...
module 2x2_static_stagger_full_wall_line(front, back, right, left, manifold=-0.05) {
  translate([0,1,0]) scale([1,1,1]) union() {
    translate([1,0,0]) cube([48,48,7]);
    translate([0,-manifold,0]) union() {
...
module 2x2_wall_top(seed, manifold=-0.05) {
  seed_vect = rands(0, 100, 32, seed=seed);

  translate([0,25,0]) rotate([0,0,0]) union() {
...
module 2x2_static_wall_top(top, manifold=-0.05) {
  translate([0,25,0]) rotate([0,0,0]) union() {
    translate([0,1,manifold]) rotate([-90,0,0]) scale([1,1,8.2/7]) union() {
...
module 2x2_staggered_wall_top(seed, manifold=-0.05) {
  seed_vect = rands(0, 100, 32, seed=seed);
...
module 2x2_static_staggered_wall_top(top, manifold=-0.05) {
  translate([25,0,0]) rotate([0,0,90]) union() {
    translate([0,1,manifold]) rotate([-90,0,0]) scale([1,1,8.2/7]) union() {
      translate([1,0,-21.3]) scale([.9,1,6/10]) static_face("../../../lib/", top[0]);
      translate([10,0,-21.3]) scale([1,1,6/10]) static_face("../../../lib/", top[1]);
...
if(rend) {
  if(random) {
    if(staggered) {
      2x2_stagger_full_wall_line(0, seed_vect[0]);
      if(render_top) {
        translate([0,0,7]) 2x2_staggered_wall_top(seed_vect[1]);
      }
    } else {
      2x2_full_wall_line(0, seed_vect[0]);
      if(render_top) {
        translate([0,0,7]) 2x2_wall_top(seed_vect[1]);
      }
    }
  } else {
    if(staggered) {
      2x2_static_stagger_full_wall_line(front, back, right, left);
      if(render_top) {
        translate([0,0,7]) 2x2_static_staggered_wall_top(top);
      }
    } else {
      2x2_static_full_wall_line(front, back, right, left);
      if(render_top) {
        translate([0,0,7]) 2x2_static_wall_top(top);
      }
    }
  }
}
